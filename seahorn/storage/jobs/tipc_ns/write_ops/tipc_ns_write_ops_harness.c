...
int main(void) {
  ...
  size_t sz = nd_size_t();
  assume(sz < sea_max_buffer_size());
  size_t required_bytes = sizeof(uint16_t) * sz;
  uint16_t *data = malloc(required_bytes);

  ns_read_pos(c_chan, handle, nd_uint64_t(), data, required_bytes);
